...
export async function GET(request: NextRequest) {
  ...
  if (carId && carId !== "all") {
    ...
    transactions = rawTransactions.map((trx) => {
      const itemsTotal =
        trx.expense?.items.reduce(
  ...
  doc.text("-", 43 + colWidths[0] + colWidths[1], y + 4, {
    width: colWidths[2] - 6,
  });
  doc.text("-", 43 + colWidths[0] + colWidths[1] + colWidths[2], y + 4, {
    width: colWidths[3] - 6,
    align: "right",
  });
  doc.text(
    "-",
    43 + colWidths[0] + colWidths[1] + colWidths[2] + colWidths[3],
    y + 4,
    ...
    async function worker() {
      while (true) {
        ...
        try {
          ...
          if (!response.ok) {
            console.warn(
              "[reports/finance] receipt fetch failed",
              receipt.url,
              response.status,
            );
          ...
          console.warn(
            "[reports/finance] receipt fetch error",
            receipt.url,
            err,
          );
  ...
  for (const trx of transactions) {
    ...
    if (trx.expense?.items && trx.expense.items.length > 0) {
      ...
      if (trx.type === TransactionType.INCOME) {
        doc.text(formatRupiah(trx.amount), x, y + 4, {
          width: colWidths[3] - 6,
          align: "right",
        });
        x += colWidths[3];
  ...
  if (receipts.length > 0) {
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("Lampiran Nota", { align: "center" });
    doc.moveDown(1.5);
    ...
    for (const result of receiptResults) {
      ...
      if (result.ok && result.buffer) {
        ...
        doc
          .fontSize(9)
          .font("Helvetica")
          .text(`[Gambar tidak tersedia]`, { align: "center" });
        doc.moveDown(2);
      }
