...
import { TransactionType } from '../src/generated/prisma';
...
async function verifyMigration(filepath: string) {
  ...
  try {
    ...
    const expectedTrx = exportData.incomes.length + exportData.expenses.length + exportData.fuelUsages.length;
    results.push({
      model: 'Transactions',
      old: expectedTrx,
    ...
    const usageWithUser = (exportData.usageRecords as { userId: string | null }[]).filter((r) => r.userId).length;
    results.push({
      model: 'Usage Records',
      old: usageWithUser,
      new: usageCount,
    ...
    results.push({
      ...
      status: pengajuanItemCount >= exportData.pengajuanItems.length ? '✅' : '⚠️',
    });
    ...
    for (const r of results) {
      console.log(`| ${r.model.padEnd(16)} | ${String(r.old).padStart(6)} | ${String(r.new).padStart(6)} | ${r.status}     |`);
    }
    ...
    if (lastTrx) {
      ...
      console.log(`   Current Balance (stored):     ${storedBalance.toLocaleString('id-ID')}`);
      console.log(`   Current Balance (calculated): ${calculatedBalance.toLocaleString('id-ID')}`);
...
if (!filepath) {
  console.error('Usage: npx tsx scripts/verify-migration.ts <path-to-export.json>');
  process.exit(1);
}
